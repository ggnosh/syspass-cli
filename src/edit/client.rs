...
pub fn command_helper() -> Command {
    Command::new(COMMAND_NAME)
        .about("Edit client")
        .arg(arg!(-i --id <ID> "Client ID").required(false).value_parser(clap::value_parser!(u32)))
...
fn edit_client(matches: &ArgMatches, api_client: &dyn ApiClient, id: u32, quiet: bool) -> Result<u8, Box<dyn Error>>
{
    ...
    client.description = get_match_string(matches, quiet, "description", "Description: ",&client.description, false);

    info!("Trying to edit client");

    match api_client.save_client(&client) {
        Ok(client) => {
            warn!("{} Client {} ({}) saved!", "\u{2714}".bright_green(), client.name.green(), client.id.unwrap());
            Ok(0)
        },
        Err(error) => {
            error!("{} Could not save client\n{}", "\u{2716}".bright_red(), error);
            Err(error)?
        }
    }
}
