...
pub fn command_helper() -> Command {
    Command::new(COMMAND_NAME)
        ...
        .arg(arg!(-i --client <CLIENTID> "Client id").required(false).value_parser(clap::value_parser!(u32)))
        .arg(arg!(-a --category <CATEGORYID> "Category id").required(false).value_parser(clap::value_parser!(u32)))
        .arg(arg!(-g --global <INT> "Should the client be global or not").required(false).value_parser(clap::value_parser!(usize)))
...
pub fn command(matches: &ArgMatches, api_client: &dyn ApiClient, quiet: bool) -> Result<u8, Box<dyn Error>>
{
    let account: Account = Account {
        ...
        category_id: matches.get_one::<u32>("category").map(|s| s.to_owned()).unwrap_or_else(|| {
            if quiet {
        ...
        client_id: matches.get_one::<u32>("client").map(|s| s.to_owned()).unwrap_or_else(|| {
            if quiet {
                warn!("Could not ask for client");
                process::exit(1);
            }
        ...
        pass: Option::from(matches.get_one::<String>("password").map(|s| s.to_string()).unwrap_or_else(|| {
            if quiet {
                warn!("Could not ask for client");
                process::exit(1);
            }
    ...
    match api_client.save_account(&account) {
        Ok(account) => {
            warn!("{} Account {} ({}) saved!", "\u{2714}".bright_green(), account.name.green(), account.id.unwrap());
            Ok(0)
        },
        Err(error) => Err(format!("{} Could not save client: {}", error, "\u{2716}".bright_red()))?
    }
}
