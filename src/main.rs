...
fn main() -> Result<ExitCode, Box<dyn Error>>
{
    let matches = Command::new(crate_name!())
        ...
        .arg(arg!(-c --config <FILE> "Sets a custom config file").global(true).required(false).display_order(100))
        .arg(arg!(-q --quiet "Do not output any message").global(true).required(false).display_order(100))
        .arg(arg!(-v --verbose "Output more information").global(true).required(false).display_order(100))
        .arg(arg!(-d --debug "Output debug information").global(true).required(false).display_order(100))
    ...
    let api_client_box: Box<dyn ApiClient> = Api::from_str(api_version).unwrap_or_else(|_| {
        panic!("No such API is supported ({})", &api_version)
    ...
    log::set_logger(&LOGGER).map(|()| log::set_max_level(log_level)).expect("Failed to set logger");
